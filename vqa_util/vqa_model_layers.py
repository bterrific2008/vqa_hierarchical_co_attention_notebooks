...
class AttentionMaps(tf.keras.layers.Layer):
    ...
    def call(self, image_feat, ques_feat):
        ...
        # (QT)(Wb)V
        C = tf.matmul(
            ques_feat, tf.transpose(image_feat, perm=[0, 2, 1])
        )  # [b, 23, 49]
        # tanh((QT)(Wb)V)
        ...
        # ((Wv)V)CT                                           # [b, k, 23]
        WvV_C = tf.matmul(
            tf.transpose(WvV, perm=[0, 2, 1]), tf.transpose(C, perm=[0, 2, 1])
        )
    ...
    def get_config(self):
        ...
        """
        config = {"dim_k": self.dim_k, "reg_value": self.reg_value}
...
class PhraseLevelFeatures(tf.keras.layers.Layer):
    ...
    def call(self, word_feat):
        ...
        x = tf.concat(
            [
                tf.expand_dims(x_uni, -1),
                tf.expand_dims(x_bi, -1),
                tf.expand_dims(x_tri, -1),
            ],
            -1,
        )  # [b, 23, dim_d, 3]
