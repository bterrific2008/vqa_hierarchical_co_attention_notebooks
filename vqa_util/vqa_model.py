...
from .vqa_model_layers import AttentionMaps, ContextVector, PhraseLevelFeatures


def build_model(
    max_answers, max_seq_len, vocab_size, dim_d, dim_k, l_rate, d_rate, reg_value
):
    """
    ...
    # word level
    ques_feat_w = Embedding(
        input_dim=vocab_size, output_dim=dim_d, input_length=max_seq_len, mask_zero=True
    )(ques_input)

    Hv_w, Hq_w = AttentionMaps(dim_k, reg_value, name="AttentionMaps_Word")(
        image_feat, ques_feat_w
    )
    v_w, q_w = ContextVector(reg_value, name="ContextVector_Word")(
        image_feat, ques_feat_w, Hv_w, Hq_w
    )
    ...
    Hv_p, Hq_p = AttentionMaps(dim_k, reg_value, name="AttentionMaps_Phrase")(
        image_feat, ques_feat_p
    )
    v_p, q_p = ContextVector(reg_value, name="ContextVector_Phrase")(
        image_feat, ques_feat_p, Hv_p, Hq_p
    )
    ...
    h_p = Dense(
        ...
        name="h_p_Dense",
        kernel_regularizer=tf.keras.regularizers.l2(reg_value),
        kernel_initializer=tf.keras.initializers.glorot_uniform(seed=14),
    )(feat_p)
    ...
    ques_feat_s = LSTM(
        ...
        input_shape=(None, max_seq_len, dim_d),
        kernel_initializer=tf.keras.initializers.glorot_uniform(seed=16),
    )(ques_feat_p)

    Hv_s, Hq_s = AttentionMaps(dim_k, reg_value, name="AttentionMaps_Sent")(
        image_feat, ques_feat_s
    )
    v_s, q_s = ContextVector(reg_value, name="ContextVector_Sent")(
        image_feat, ques_feat_p, Hv_s, Hq_s
    )
    ...
    h_s = Dense(
        ...
        name="h_s_Dense",
        kernel_regularizer=tf.keras.regularizers.l2(reg_value),
        kernel_initializer=tf.keras.initializers.glorot_uniform(seed=15),
    )(feat_s)
